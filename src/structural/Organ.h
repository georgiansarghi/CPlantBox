...
class Organ : public std::enable_shared_from_this<Organ> {
  ...
  public:
    ...
    virtual double calcAge(double length) const;                                   ///< Maps length -> age; must be overridden; needed by getOrgans()
    virtual double calcLength(double age);                                         ///< Maps age -> length; must be overridden
